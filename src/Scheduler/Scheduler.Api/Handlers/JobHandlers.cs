    ...
    IMessageHandler<JobQuery, JobModel>
{
    ...
    {
        var model = new JobTriggerEventModel(
            id: Guid.NewGuid(),
            command.Name,
    ...
    public async Task Handle(JobTriggerUpdateCommand command, CancellationToken token) =>
        await storage.AddOrUpdate(
            command.Id,
            addFactory: _ => throw new NotFoundException(),
            updateFactory: (_, _) => new JobTriggerEventModel(command.Id, command.Name, command.TriggerEventName, command.TriggerEventMask),
            token);
